...
@app.route("/cac-utils/favicon.ico")
def favicon_ico():
    return send_from_directory(
        os.path.dirname(os.path.abspath(__file__)), "favicon.ico"
    )
...
def get_data_from_headers():
    ...
    verify_status = request.headers.get("X-Ssl-Client-Verify") or request.headers.get(
        "Ssl-Client-Verify"
    )
    ...
    cert_raw = request.headers.get("X-Ssl-Client-Cert") or request.headers.get(
        "Ssl-Client-Cert"
    )
    if cert_raw:
        try:
    ...
    client_dn = request.headers.get("X-Ssl-Client-Dn") or request.headers.get(
